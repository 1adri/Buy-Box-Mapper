...
function setUiMode(mode, shouldPersist = true) {
  ...
  $('btnStandardMode').classList.toggle('active', uiMode === 'standard');
  $('btnAdvancedMode').classList.toggle('active', uiMode === 'advanced');
  if (shouldPersist) saveInputs();
}
...
function saveInputs() {
  chrome.storage.local.set({ bbgs_inputs: {
    ...
    mode: $('mode').value,
    captchaStreakLimit: $('captchaStreakLimit').value,
    captchaCooldownSec: $('captchaCooldownSec').value,
    uiMode
  }});
}
...
$('btnStart').addEventListener('click', () => {
  ...
  const captchaStreakLimit = Math.max(1, parseInt($('captchaStreakLimit').value, 10) || 3);
  const captchaCooldownSec = Math.max(0, parseInt($('captchaCooldownSec').value, 10) || 0);
